...
if (listaCandidatos.length)
    recorrerListaCandidatos(listaCandidatos, listaDirectorios, 0)
else
 ...
 **/
async function recorrerListaCandidatos(candidatos, directorios, index) {
    if (candidatos.length === index) return
    await ejecutarCandidato(candidatos[index], directorios[index], index)
    await recorrerListaCandidatos(candidatos, directorios, index + 1)
}
